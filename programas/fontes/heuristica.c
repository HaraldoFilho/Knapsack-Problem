...
void print_help(){
	...
	printf("uso: heuristica -f <entrada> -t <tempomax> -o <saida>\n\n");
	printf("onde: <entrada>  = arquivo de entrada\n");
	printf("      <tempomax> = tempo maximo de execucao em segundos\n");
	printf("      <saida>    = arquivo de saida\n\n");
	printf("exemplos: heuristica -f entrada.txt -t 60 -o saida.txt\n\n");
	
}

int main(int argc, char *argv[]) {

	if ((argc!=7)||(strcmp(argv[1],"-f")!=0)||(strcmp(argv[3],"-t")!=0)||(strcmp(argv[5],"-o")!=0)) {
		print_help();
		return 0;
	}
	...
	printf("\nResolvendo o problema por conjunto de heuristicas...\n");
	data_out_str r = solve_by_heuristics(d, tmax, INFO_ON);
